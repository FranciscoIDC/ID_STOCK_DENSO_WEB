(function () {
    ...
    angular
        .module('app')
        .factory('DestinationEditService', DestinationEditService);

    DestinationEditService.$inject = ['$http', 'UrlService'];

    function DestinationEditService($http, UrlService) {
        let service = {
            Get: _Get,
            Update: _Update
        };
        ...
        function _Get(id) {
            let url = UrlService.getDestinationById.replace("{id}", id)
            let request = {
                method: 'GET',
                url: url,
                async: true
