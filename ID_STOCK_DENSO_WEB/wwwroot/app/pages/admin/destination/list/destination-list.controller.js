(function () {
    ...
    DestinationListController.$inject = [
        '$scope',
        '$state',
        'DestinationListService',
        'AlertService',
        'ErrorService',
        'ConstService',
        'GeneralInfo'
    ...
    function DestinationListController(
        ...
        async function _GetAssetsOnResume() {
            let filters = JSON.parse(localStorage.getItem(ConstService.destinationListFilters));
            if (!filters) return;
        ...
        async function _Search(newPage) {
            ...
            localStorage.setItem(ConstService.destinationListFilters, JSON.stringify(filters))
            ...
            await _GetDestinationAsync(params);
            swal.close();
        }

        async function _GetDestinationAsync(params) {
            try {
                self.data = [];
                self.totalItems = 0;
                self.counterData = 0;
                let response = await DestinationListService.Get(params);
        ...
        async function _PrintTagAsync(model) {
            try {
                ...
                let dataSend = { 'JsonObject': JSON.stringify(model), 'LabelType': 'destination' }
                let response = await DestinationListService.Print(dataSend, adminId);
                const { Status: status, Message: message, Data: data, Counter: counter } = response.data;
                console.log(response);
        ...
        function _GoEdit(id, addsId) {
            $state.go("destination.edit", { "id": id });
        }
        ...
        function _ChangueCurrentPage(newPage, oldPage) {
            ...
            _GetDestinationAsync(params);
            swal.close();
        }

        function _ClearFilters() {
            localStorage.removeItem(ConstService.destinationListFilters);
            self.IdLocation = "";
            self.categoryId = "";
            self.companyId = "";
        }
