(function () {
    ...
    angular
        .module('app')
        .factory('FarmerEditService', FarmerEditService);

    FarmerEditService.$inject = ['$http', 'UrlService'];

    function FarmerEditService($http, UrlService) {
        let service = {
            Get: _Get,
            Update: _Update
        };
        ...
        function _Get(id) {
            let url = UrlService.getFarmerById.replace("{id}", id)
            let request = {
                method: 'GET',
                url: url,
                async: true
