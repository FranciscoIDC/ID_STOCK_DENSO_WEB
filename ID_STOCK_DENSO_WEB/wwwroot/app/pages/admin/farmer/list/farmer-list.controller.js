(function () {
    angular
        .module('app')
        .controller('FarmerListController', FarmerListController);

    FarmerListController.$inject = [
        '$scope',
        '$state',
        'FarmerListService',
        'AlertService',
    ...
    function FarmerListController(
        ...
        async function _GetAssetsOnResume() {
            let filters = JSON.parse(localStorage.getItem(ConstService.farmerListFilters));
            if (!filters) return;
        ...
        async function _Search(newPage) {
            ...
            localStorage.setItem(ConstService.farmerListFilters, JSON.stringify(filters))
            ...
            await _GetFarmerAsync(params);
            swal.close();
        }

        async function _GetFarmerAsync(params) {
            try {
                self.data = [];
                self.totalItems = 0;
                self.counterData = 0;
                let response = await FarmerListService.Get(params);
        ...
        async function _PrintTagAsync(model) {
            try {
                ...
                let dataSend = { 'JsonObject': JSON.stringify(model), 'LabelType': 'farmer' }
                let response = await FarmerListService.Print(dataSend, adminId);
                const { Status: status, Message: message, Data: data, Counter: counter } = response.data;
                console.log(response);
        ...
        function _ChangueCurrentPage(newPage, oldPage) {
            ...
            _GetFarmerAsync(params);
            swal.close();
        }

        function _ClearFilters() {
            localStorage.removeItem(ConstService.farmerListFilters);
            self.IdLocation = "";
            self.categoryId = "";
            self.companyId = "";
        }
