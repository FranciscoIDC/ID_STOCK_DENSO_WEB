(function () {
    angular
        .module('app')
        .controller('SizeListController', SizeListController);

    SizeListController.$inject = [
        '$scope',
        '$state',
        'SizeListService',
        'AlertService',
    ...
    function SizeListController(
        ...
        async function _GetAssetsOnResume() {
            let filters = JSON.parse(localStorage.getItem(ConstService.sizeListFilters));
            if (!filters) return;
        ...
        async function _Search(newPage) {
            ...
            localStorage.setItem(ConstService.sizeListFilters, JSON.stringify(filters))
            ...
            await _GetSizeAsync(params);
            swal.close();
        }

        async function _GetSizeAsync(params) {
            try {
                self.data = [];
                self.totalItems = 0;
                self.counterData = 0;
                let response = await SizeListService.Get(params);
        ...
        async function _PrintTagAsync(model) {
            try {
                ...
                let dataSend = { 'JsonObject': JSON.stringify(model), 'LabelType': 'size' }
                let response = await SizeListService.Print(dataSend, adminId);
                const { Status: status, Message: message, Data: data, Counter: counter } = response.data;
                console.log(response);
        ...
        function _ChangueCurrentPage(newPage, oldPage) {
            ...
            _GetSizeAsync(params);
            swal.close();
        }

        function _ClearFilters() {
            localStorage.removeItem(ConstService.sizeListFilters);
            self.IdLocation = "";
            self.categoryId = "";
            self.companyId = "";
        }
