(function () {
    ...
    IncidentReportController.$inject = [
        '$scope',
        '$state',
    ...
    function IncidentReportController(
        self,
        $state,
        IncidentReportService,
        AlertService,
        ...
        async function _GetAssetsOnResume() {
            ...
            let params = BuildParamsToGetIncident(self.currentPage, self.itemsPerPage, self.startDate, self.endDate);
            await _GetIncidentAsync(params)
        }
        ...
        function Filters() {
            var dt1 = GeneralInfo.formatDate(self.startDate);
            var dt2 = GeneralInfo.formatDate(self.endDate);
            let filters = { 'DateStart': dt1, 'DateEnd': dt2 };
            localStorage.setItem(ConstService.inventoryListFilters, JSON.stringify(filters));
        }

        async function _GetIncidentAsync(params) {
            try {
                self.data = [];
                self.totalItems = 0;
                self.counterData = 0;

                let response = await IncidentReportService.Get(params);
        ...
        async function _Get_toExcel(params) {
            try {
                ...
                request.open("GET", UrlService.getIncidentReport + '/Excel?' + params);
                ...
                request.onload = function () {
                    ...
                    a.download = this.response.name || "Reporte_Incidencias" + cadenaFecha;
                    a.click();
                }
                request.send();
            } catch (ex) {
        ...
        function BuildParamsToGetIncident(page, itemsPerPage, dtStart, dtEnd) {
            var dt1 = GeneralInfo.formatDate(dtStart);
            var dt2 = GeneralInfo.formatDate(dtEnd);

            //self.model = new IncidentDtoGet(); _ChangueItemsPerPage
        ...
        function _GoEmail(id, addsId) {
            $state.go("incident.report");
        }
