(function () {
    angular
        .module('app')
        .controller('OutboundReportController', OutboundReportController);

    OutboundReportController.$inject = [
        '$scope',
        '$state',
        '$timeout',
        'OutboundReportService',
        'AlertService',
        'ErrorService',
        'ConstService',
        'GeneralInfo',
    ...
    function OutboundReportController(
        self,
        $state,
        $timeout,
        OutboundReportService,
        ...
        function _RegisterFunctions() {
            ...
            self.HasSignature = _HasSignature;
        ...
        async function _GetOutboundAsync(params) {
            try {
                self.data = [];
                self.totalItems = 0;
                self.counterData = 0;

                let response = await OutboundReportService.Get(params);
                const {
                    Status: status,
                    Message: message,
                    Data: data,
        ...
        function _OpenDetailModal(order) {
            ...
            $timeout(function () {
                $('#modalDetailOutbound').modal('show');
            });
        }
        ...
        function _HasSignature(order) {
            return order && order.SignaturePhoto && order.SignaturePhoto.trim() !== '';
        }

        // ========== SEMÁFORO ==========
        // PasoActual: 0=sin mov, 1=picking, 2=entregado
        function _GetPasoClass(paso) {
            switch (paso) {
                case 2: return 'badge-status-completed';
                case 1: return 'badge-status-in-progress';
        ...
        function _GetPasoLabel(paso) {
            switch (paso) {
                ...
                case 1: return 'En Picking';
                default: return 'Sin movimiento';
            }
        }

        // stepIndex: 1=Picking, 2=Entrega
        ...
        function _GetCSV() {
            ...
            let headers = [
                '# Requisición', 'No. Requisición', 'Solicitante', 'F. Solicitud',
                'Picking - Usuario', 'Picking - Fecha', 'Picking - Cant. Total',
                'Entrega - Usuario', 'Entrega - Fecha', 'Tiene Firma',
                'Paso Actual',
                'No. Parte', 'No. Denso', 'Denso N', 'Descripción',
                'Tipo', 'Cant. Solicitada'
            ];
            ...
            a.download = 'Reporte_Salidas_' + fecha + '.csv';
            a.click();
        ...
        function _BuildCsvRow(row, part) {
            ...
            let cols = [
                esc(row.IdRequisitionOrder),
                esc(row.RequisitionNumber),
                esc(row.Requester),
                esc(_FormatDate(row.RequestDate)),
                esc(row.PickingUser),
                esc(_FormatDateTime(row.PickingDateTime)),
                esc(row.PickingQuantity),
                esc(row.DeliveryUser),
                esc(_FormatDateTime(row.DeliveryDateTime)),
                esc(_HasSignature(row) ? 'Sí' : 'No'),
