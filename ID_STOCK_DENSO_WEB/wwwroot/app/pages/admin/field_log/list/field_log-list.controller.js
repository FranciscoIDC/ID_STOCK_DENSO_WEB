(function () {
    angular
        .module('app')
        .controller('FieldLogListController', FieldLogListController);

    FieldLogListController.$inject = [
        '$scope',
        '$state',
        'FieldLogListService',
        'AlertService',
    ...
    function FieldLogListController(
        self,
        $state,
        FieldLogListService,
        AlertService,
        ...
        function _RegisterFunctions() {
            ...
            self.CloseImageModal = _CloseImageModal;
            self.GoToImageEditor = _GoToImageEditor;
        }
        ...
        async function _GetAssetsOnResume() {
            let filters = JSON.parse(localStorage.getItem(ConstService.fieldLogListFilters));
            if (!filters) return;
        ...
        async function _Search(newPage) {
            ...
            localStorage.setItem(ConstService.fieldLogListFilters, JSON.stringify(filters))
            ...
            await _GetFieldLogAsync(params);
            swal.close();
        }

        async function _GetFieldLogAsync(params) {
            try {
                self.data = [];
                self.totalItems = 0;
                self.counterData = 0;
                let response = await FieldLogListService.Get(params);
                ...
                // 1) Formatear fechas
                const rawWithDates = _FormatAllDates(data);

                // 3) Agrupar por viaje
                const grouped = _GroupFieldLogs(rawWithDates);
        ...
        function _GroupFieldLogs(data) {
            if (!data || !Array.isArray(data)) return [];

            const map = {};

            data.forEach(item => {
                const key = (item.Id !== undefined ? item.Id : '') + '|' + (item.TripNumber || '');
                ...
                if (!map[key]) {
                    map[key] = {
                        ...
                        RemissionPhoto: item.RemissionPhoto,
                        ...
                        ArrivalSeal: item.ArrivalSeal,
                        ArrivalSealPhoto: item.ArrivalSealPhoto,
                        ScaleOutPhoto: item.ScaleOutPhoto,
                        TripStatusName: item.TripStatusName,
                        IdTripStatus: item.IdTripStatus,
                        TonCost: item.TonCost,
                        FreightFolio: item.FreightFolio,
                        Percentaje1era: item.Percentaje1era,
                        SoilSampleWeight: item.SoilSampleWeight,
                        SoilSamplePhoto: item.SoilSamplePhoto,
                        Details: []
                    };
                }
            ...
            const trips = Object.values(map).map(trip => {
                ...
                trip.Details = _FormatAllNumbers(trip.Details, 2);
        ...
        function _FormatAllNumbers(data, decimals = 2) {
            if (!data) return data;

            // Si es array, recorre cada elemento
            if (Array.isArray(data)) {
                return data.map(item => _FormatAllNumbers(item, decimals));
            }

            // Si es objeto, recorre sus propiedades
            if (typeof data === 'object') {
                for (const key in data) {
                    ...
                    if (Array.isArray(value) || (typeof value === 'object' && value !== null)) {
                        // Llamada recursiva para objetos/arrays anidados
                        data[key] = _FormatAllNumbers(value, decimals);
                    } else if (typeof value === 'number' || (!isNaN(value) && value !== '' && value !== null)) {
                        data[key] = _FormatNumber(value, decimals);
                    }
                }
            }
        ...
        async function _PrintTagAsync(model) {
            try {
                ...
                let dataSend = { 'JsonObject': JSON.stringify(model), 'LabelType': 'fieldLog' }
                let response = await FieldLogListService.Print(dataSend, adminId);
                const { Status: status, Message: message, Data: data, Counter: counter } = response.data;
                console.log(response);
        ...
        function _ChangueCurrentPage(newPage, oldPage) {
            ...
            _GetFieldLogAsync(params);
            swal.close();
        }

        function _OpenImageModal(key, title, url) {
        ...
        function _CloseImageModal() {
            ...
            self.currentImageKey = null;
            self.currentImageUrl = null;
            self.currentImageTitle = "";
        }

        // ✅ Redirección a otra vista (ejemplo: "image.edit")
        function _GoToImageEditor(key) {
            self.CloseImageModal();
            $state.go("image.edit", { "imageKey": key });
        }

        function _ClearFilters() {
            localStorage.removeItem(ConstService.fieldLogListFilters);
            self.IdLocation = "";
            self.categoryId = "";
            self.companyId = "";
        }
