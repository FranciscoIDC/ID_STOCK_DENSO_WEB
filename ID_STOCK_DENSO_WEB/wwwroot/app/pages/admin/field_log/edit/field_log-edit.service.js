(function () {
    ...
    angular
        .module('app')
        .factory('FieldLogEditService', FieldLogEditService);

    FieldLogEditService.$inject = ['$http', 'UrlService'];

    function FieldLogEditService($http, UrlService) {
        let service = {
            Get: _Get,
            Update: _Update
        };
        ...
        function _Get(id) {
            let url = UrlService.getFieldLogById.replace("{id}", id)
            let request = {
                method: 'GET',
                url: url,
                async: true
