(function () {
    angular
        .module('app')
        .controller('RejectionListController', RejectionListController);

    RejectionListController.$inject = [
        '$scope',
        '$state',
    ...
    function RejectionListController(
        self,
        $state,
        RejectionListService,
        AlertService,
        ...
        async function _GetAssetsOnResume() {
            let filters = JSON.parse(localStorage.getItem(ConstService.rejectionListFilters));
            if (!filters) return;
        ...
        async function _Search(newPage) {
            ...
            localStorage.setItem(ConstService.rejectionListFilters, JSON.stringify(filters))
            ...
            await _GetRejectionAsync(params);
            swal.close();
        }

        async function _GetRejectionAsync(params) {
            try {
                self.data = [];
                self.totalItems = 0;
                self.counterData = 0;
                let response = await RejectionListService.Get(params);
        ...
        async function _PrintTagAsync(model) {
            try {
                ...
                let dataSend = { 'JsonObject': JSON.stringify(model), 'LabelType': 'rejection' }
                let response = await RejectionListService.Print(dataSend, adminId);
                const { Status: status, Message: message, Data: data, Counter: counter } = response.data;
                console.log(response);
        ...
        function _ChangueCurrentPage(newPage, oldPage) {
            ...
            _GetRejectionAsync(params);
            swal.close();
        }

        function _ClearFilters() {
            localStorage.removeItem(ConstService.rejectionListFilters);
            self.IdLocation = "";
            self.categoryId = "";
            self.companyId = "";
        }
