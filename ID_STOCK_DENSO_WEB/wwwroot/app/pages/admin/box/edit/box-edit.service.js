(function () {
    ...
    angular
        .module('app')
        .factory('BoxEditService', BoxEditService);

    BoxEditService.$inject = ['$http', 'UrlService'];

    function BoxEditService($http, UrlService) {
        let service = {
            Get: _Get,
            Update: _Update
        };
        ...
        function _Get(id) {
            let url = UrlService.getBoxById.replace("{id}", id)
            let request = {
                method: 'GET',
                url: url,
                async: true
