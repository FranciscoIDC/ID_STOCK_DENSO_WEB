(function () {
    angular
        .module('app')
        .controller('BoxListController', BoxListController);
    ...
    function BoxListController(
        ...
        async function _GetAssetsOnResume() {
            let filters = JSON.parse(localStorage.getItem(ConstService.boxListFilters));
            if (!filters) return;
        ...
        async function _Search(newPage) {
            ...
            localStorage.setItem(ConstService.boxListFilters, JSON.stringify(filters))
            ...
            await _GetBoxAsync(params);
            swal.close();
        }

        async function _GetBoxAsync(params) {
            try {
                self.data = [];
                self.totalItems = 0;
                self.counterData = 0;
                let response = await BoxListService.Get(params);
        ...
        async function _PrintTagAsync(model) {
            try {
                ...
                let dataSend = { 'JsonObject': JSON.stringify(model), 'LabelType': 'box' }
                let response = await BoxListService.Print(dataSend, adminId);
                const { Status: status, Message: message, Data: data, Counter: counter } = response.data;
                console.log(response);
        ...
        function _ClearFilters() {
            localStorage.removeItem(ConstService.boxListFilters);
            self.IdLocation = "";
            self.categoryId = "";
            self.companyId = "";
        }
