(function () {
    angular
        .module('app')
        .controller('VarietyListController', VarietyListController);

    VarietyListController.$inject = [
        '$scope',
        '$state',
        'VarietyListService',
        'AlertService',
    ...
    function VarietyListController(
        self,
        $state,
        VarietyListService,
        AlertService,
        ...
        async function _GetAssetsOnResume() {
            let filters = JSON.parse(localStorage.getItem(ConstService.varietyListFilters));
            if (!filters) return;
        ...
        async function _Search(newPage) {
            ...
            localStorage.setItem(ConstService.varietyListFilters, JSON.stringify(filters))
            ...
            await _GetVarietyAsync(params);
            swal.close();
        }

        async function _GetVarietyAsync(params) {
            try {
                self.data = [];
                self.totalItems = 0;
                self.counterData = 0;
                let response = await VarietyListService.Get(params);
        ...
        async function _PrintTagAsync(model) {
            try {
                ...
                let dataSend = { 'JsonObject': JSON.stringify(model), 'LabelType': 'variety' }
                let response = await VarietyListService.Print(dataSend, adminId);
                const { Status: status, Message: message, Data: data, Counter: counter } = response.data;
                console.log(response);
        ...
        function _ChangueCurrentPage(newPage, oldPage) {
            ...
            _GetVarietyAsync(params);
            swal.close();
        }

        function _ClearFilters() {
            localStorage.removeItem(ConstService.varietyListFilters);
            self.IdLocation = "";
            self.categoryId = "";
            self.companyId = "";
        }
