(function () {
    ...
    angular
        .module('app')
        .factory('VarietyEditService', VarietyEditService);

    VarietyEditService.$inject = ['$http', 'UrlService'];

    function VarietyEditService($http, UrlService) {
        let service = {
            Get: _Get,
            Update: _Update
        };
        ...
        function _Get(id) {
            let url = UrlService.getVarietyById.replace("{id}", id)
            let request = {
                method: 'GET',
                url: url,
                async: true
