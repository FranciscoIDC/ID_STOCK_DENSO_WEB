(function () {
    ...
    angular
        .module('app')
        .factory('DriverEditService', DriverEditService);

    DriverEditService.$inject = ['$http', 'UrlService'];

    function DriverEditService($http, UrlService) {
        let service = {
            Get: _Get,
            Update: _Update
        };
        ...
        function _Get(id) {
            let url = UrlService.getDriverById.replace("{id}", id)
            let request = {
                method: 'GET',
                url: url,
                async: true
