(function () {
    ...
    function InventoryListController(
        ...
        function _InitValues() {
            ...
            self.model = new InventoryDtoGet();
        ...
        async function _GetAssetsOnResume() {
            ...
            let params = BuildParamsToGetInventory(self.currentPage, self.itemsPerPage, self.IdZone, self.startDate, self.endDate);
            await _GetInventoryAsync(params)
        }
        ...
        async function _Search() {
            ...
            let params = BuildParamsToGetInventory(self.currentPage, self.itemsPerPage, self.IdZone, self.startDate, self.endDate);
            //let params = { 'Page': self.currentPage, 'ItemsPerPage': self.itemsPerPage, 'LocationId': self.IdZone, 'CategoryId': self.categoryId, 'Search': self.filterSearch }
            await _GetInventoryAsync(params);
            swal.close();
        }

        async function _GetInventoryAsync(params) {
            try {
                self.data = [];
                self.totalItems = 0;
                self.counterData = 0;

                //let params = { 'Page': self.currentPage, 'ItemsPerPage': self.itemsPerPage, 'IdZone': self.IdZone, 'DtStart': self.startDate, 'DtEnd': self.endDate }
        ...
        async function _Get_toExcel(params) {
            try {
                ...
                request.open("GET", UrlService.getInventory + '/Excel?' + params);
        ...
        function BuildParamsToGetInventory(page, itemsPerPage, localizationId, dtStart, dtEnd) {
            var dt1 = GeneralInfo.formatDate(dtStart);
            var dt2 = GeneralInfo.formatDate(dtEnd);

            self.model = new InventoryDtoGet(); _ChangueItemsPerPage
            self.model.Page = page;
            self.model.ItemsPerPage = +itemsPerPage;
            self.model.IdZone = +localizationId;
        ...
        async function _GetZoneListAsync() {
            try {
                let params = { 'Page': 0, 'ItemsPerPage': 0, 'IdWarehouse': 0, }
        ...
        function _ChangueCurrentPage(newPage, oldPage) {
            ...
            AlertService.Load();
            //let params = { 'Page': newPage, 'ItemsPerPage': self.itemsPerPage, 'LocationId': self.IdZone, 'CategoryId': self.categoryId, 'Search': self.filterSearch }
            //let params = { 'Page': self.currentPage, 'ItemsPerPage': self.itemsPerPage, 'IdZone': self.IdZone, 'DtStart': self.startDate, 'DtEnd': self.endDate }
            let params = BuildParamsToGetInventory(self.currentPage, self.itemsPerPage, self.IdZone, self.startDate, self.endDate);
            _GetInventoryAsync(params);
            swal.close();
        }
        ...
        async function _ChangueItemsPerPage() {
            ...
            AlertService.Load();
            //let filters = { 'LocationId': self.IdZone, 'CompanyId': self.companyId };
        ...
        async function _GetImagesReport(id) {
            try {
                $('#CarouselImages').carousel(0);
                self.ImagesReport = [];
                let response = await InventoryListService.Getimages(id);
                const { Status: status, Message: message, Data: data, Counter: counter } = response.data;
                if (status !== 200) {
                    console.error(message);
                    AlertService.Error("Opps", message);
