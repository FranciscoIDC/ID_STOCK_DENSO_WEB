(function () {
    ...
    angular
        .module('app')
        .factory('ColdRoomEditService', ColdRoomEditService);

    ColdRoomEditService.$inject = ['$http', 'UrlService'];

    function ColdRoomEditService($http, UrlService) {
        let service = {
            Get: _Get,
            Update: _Update
        };
        ...
        function _Get(id) {
            let url = UrlService.getColdRoomById.replace("{id}", id)
            let request = {
                method: 'GET',
                url: url,
                async: true
