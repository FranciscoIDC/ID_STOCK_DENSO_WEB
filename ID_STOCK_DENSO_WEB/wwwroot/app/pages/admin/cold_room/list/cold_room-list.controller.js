(function () {
    angular
        .module('app')
        .controller('ColdRoomListController', ColdRoomListController);

    ColdRoomListController.$inject = [
        '$scope',
        '$state',
        'ColdRoomListService',
        'AlertService',
    ...
    function ColdRoomListController(
        self,
        $state,
        ColdRoomListService,
        AlertService,
        ...
        async function _GetAssetsOnResume() {
            let filters = JSON.parse(localStorage.getItem(ConstService.coldRoomListFilters));
            if (!filters) return;
        ...
        async function _Search(newPage) {
            ...
            localStorage.setItem(ConstService.coldRoomListFilters, JSON.stringify(filters))
            ...
            await _GetColdRoomAsync(params);
            swal.close();
        }

        async function _GetColdRoomAsync(params) {
            try {
                self.data = [];
                self.totalItems = 0;
                self.counterData = 0;
                let response = await ColdRoomListService.Get(params);
        ...
        async function _PrintTagAsync(model) {
            try {
                ...
                let dataSend = { 'JsonObject': JSON.stringify(model), 'LabelType': 'coldRoom' }
                let response = await ColdRoomListService.Print(dataSend, adminId);
                const { Status: status, Message: message, Data: data, Counter: counter } = response.data;
                console.log(response);
        ...
        function _ChangueCurrentPage(newPage, oldPage) {
            ...
            _GetColdRoomAsync(params);
            swal.close();
        }

        function _ClearFilters() {
            localStorage.removeItem(ConstService.coldRoomListFilters);
            self.IdLocation = "";
            self.categoryId = "";
            self.companyId = "";
        }
