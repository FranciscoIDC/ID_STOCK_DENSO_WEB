    ...
    function StaffListController(
        ...
        async function _GetStaffAsync(params) {
            try {
                ...
                self.totalItems = 0;
                self.counterData = 0;
                let response = await StaffListService.Get(params);
        ...
        async function _PrintTagAsync(model) {
            try {
                ...
                let dataSend = { 'JsonObject': JSON.stringify(model), 'LabelType': 'staff' }
                let response = await StaffListService.Print(dataSend, adminId);
                const { Status: status, Message: message, Data: data, Counter: counter } = response.data;
                console.log(response);
