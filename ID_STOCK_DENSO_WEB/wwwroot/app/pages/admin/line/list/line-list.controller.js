(function () {
    angular
        .module('app')
        .controller('LineListController', LineListController);

    LineListController.$inject = [
        '$scope',
        '$state',
        'LineListService',
        'AlertService',
    ...
    function LineListController(
        ...
        function _RegisterFunctions() {
            ...
            self.LineTypeChange = _LineTypeChange;
        ...
        function _GetAssetsOnResume() {
            let filters = JSON.parse(localStorage.getItem(ConstService.LineListFilters));
            if (!filters) return;
            self.IdLineType = filters.IdLineType;
        }
        ...
        async function _Search(newPage) {
            ...
            AlertService.Load();
            let filters = { 'IdLineType': self.IdLineType};
            localStorage.setItem(ConstService.LineListFilters, JSON.stringify(filters))
            self.currentPage = newPage;
            let params = { 'Page': self.currentPage, 'ItemsPerPage': self.itemsPerPage, 'IdLineType': ReturnZeroifnull(self.IdLineType), 'Search': self.filterSearch }
            await _GetLineAsync(params);
            swal.close();
        }

        async function _GetLineAsync(params) {
            try {
                self.data = [];
                self.counterData = 0;
                //let params = { 'Page': 0, 'ItemsPerPage': 0, 'DepartamentId': 0, 'IdLineType': self.IdLineType }
                let response = await LineListService.Get(params);
                const { Status: status, Message: message, Data: data, Counter: counter } = response.data;
                console.log(response);
        ...
        function _GoEdit(id) {
            $state.go("line.edit", { "id": id });
        }

        async function _Remove(id) {
            try {
                let response = await LineListService.Delete(id);
                const { Status: status, Message: message, Data: data, Counter: counter } = response.data;
                console.log(response);
                if (status !== 200) {
                    console.error(message);
        ...
        function _ClearFilters() {
            localStorage.removeItem(ConstService.LineListFilters);
            self.IdLineType = "";
            self.departmentId = "";
            self.IdLineType = "";
        }
        ...
        async function _LineTypeChange(id) {
            self.IdLineType = id;
            if (!self.IdLineType) return;
