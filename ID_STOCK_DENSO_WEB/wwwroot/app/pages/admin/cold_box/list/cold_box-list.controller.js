(function () {
    angular
        .module('app')
        .controller('ColdBoxListController', ColdBoxListController);

    ColdBoxListController.$inject = [
        '$scope',
        '$state',
        'ColdBoxListService',
        'AlertService',
    ...
    function ColdBoxListController(
        self,
        $state,
        ColdBoxListService,
        AlertService,
        ...
        async function _GetAssetsOnResume() {
            let filters = JSON.parse(localStorage.getItem(ConstService.coldBoxListFilters));
            if (!filters) return;
        ...
        async function _Search(newPage) {
            ...
            localStorage.setItem(ConstService.coldBoxListFilters, JSON.stringify(filters))
            ...
            await _GetColdBoxAsync(params);
            swal.close();
        }

        async function _GetColdBoxAsync(params) {
            try {
                self.data = [];
                self.totalItems = 0;
                self.counterData = 0;
                let response = await ColdBoxListService.Get(params);
        ...
        async function _PrintTagAsync(model) {
            try {
                ...
                let dataSend = { 'JsonObject': JSON.stringify(model), 'LabelType': 'coldBox' }
                let response = await ColdBoxListService.Print(dataSend, adminId);
                const { Status: status, Message: message, Data: data, Counter: counter } = response.data;
                console.log(response);
        ...
        function _ChangueCurrentPage(newPage, oldPage) {
            ...
            _GetColdBoxAsync(params);
            swal.close();
        }

        function _ClearFilters() {
            localStorage.removeItem(ConstService.coldBoxListFilters);
            self.IdLocation = "";
            self.categoryId = "";
            self.companyId = "";
        }
