(function () {
    ...
    angular
        .module('app')
        .factory('ColdBoxEditService', ColdBoxEditService);

    ColdBoxEditService.$inject = ['$http', 'UrlService'];

    function ColdBoxEditService($http, UrlService) {
        let service = {
            Get: _Get,
            Update: _Update
        };
        ...
        function _Get(id) {
            let url = UrlService.getColdBoxById.replace("{id}", id)
            let request = {
                method: 'GET',
                url: url,
                async: true
