(function () {
    angular
        .module('app')
        .controller('PalletAssemblyDetailController', PalletAssemblyDetailController);

    PalletAssemblyDetailController.$inject = [
        '$scope',
        '$state',
        '$stateParams',
        'PalletAssemblyDetailService',
        'AlertService',
        'ErrorService',
        'ConstService',
    ...
    function PalletAssemblyDetailController(
        self,
        $state,
        $stateParams,
        PalletAssemblyDetailService,
        ...
        function _InitValues() {
            ...
            self.model = new PalletAssemblyDetailDtoGet();

            self.IdPalletAssembly = $stateParams.id;
        ...
        async function _GetAssetsOnResume() {
            let params = BuildParamsToGetPalletAssemblyDetails(self.currentPage, self.itemsPerPage, self.IdPalletAssembly);
            await _GetPalletAssemblyDetailAsync(params)
        }
        ...
        async function _GetExcel() {
            ...
            let params = ('Page=' + 0 + '&ItemsPerPage=' + 0 +
                '&IdPalletAssembly=' + ReturnZeroifnull(self.IdPalletAssembly)
            );
        ...
        async function _Get_toExcel(params) {
            try {
                ...
                request.open("GET", UrlService.getPalletAssemblyDetail + '/Excel?' + params);
                request.onload = function () {
                    ...
                    a.download = this.response.name || "Detalle_Armado_Tarima" + cadenaFecha;
                    a.click();
                }
                request.send();
            } catch (ex) {
        ...
        async function _GetPalletAssemblyDetailAsync(params) {
            try {
                self.data = [];
                self.totalItems = 0;
                self.counterData = 0;

                let response = await PalletAssemblyDetailService.Get(params);
        ...
        function BuildParamsToGetPalletAssemblyDetails(page, itemsPerPage, IdPalletAssembly) {
            
            self.model = new PalletAssemblyDetailDtoGet();
            self.model.Page = page;
            self.model.ItemsPerPage = +itemsPerPage;
            self.model.IdPalletAssembly = +IdPalletAssembly;
        ...
        function _GoEdit(id) {
            $state.go("palletAssembly.edit", { "id": id });
        }
        ...
        function _ChangueCurrentPage(newPage, oldPage) {
            ...
            AlertService.Load();
            let params = BuildParamsToGetPalletAssemblyDetails(self.currentPage, self.itemsPerPage, self.IdPalletAssembly);
            _GetPalletAssemblyDetailAsync(params);
            swal.close();
        }
        ...
        async function _ChangueItemsPerPage() {
            AlertService.Load();
            let params = { 'Page': self.currentPage, 'ItemsPerPage': self.itemsPerPage, 'IdPalletAssembly': self.IdPalletAssembly }
            _GetPalletAssemblyDetailAsync(params);
            swal.close();
        ...
        //        self.ImagesReport = [];
        //        let response = await PalletAssemblyDetailService.Getimages(id);
        //        const { Status: status, Message: message, Data: data, Counter: counter } = response.data;
        //        if (status !== 200) {
        //            console.error(message);
        //            AlertService.Error("Opps", message);
