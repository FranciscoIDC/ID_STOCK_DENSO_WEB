(function () {
    ...
    angular
        .module('app')
        .factory('TruckEditService', TruckEditService);

    TruckEditService.$inject = ['$http', 'UrlService'];

    function TruckEditService($http, UrlService) {
        let service = {
            Get: _Get,
            Update: _Update
        };
        ...
        function _Get(id) {
            let url = UrlService.getTruckById.replace("{id}", id)
            let request = {
                method: 'GET',
                url: url,
                async: true
