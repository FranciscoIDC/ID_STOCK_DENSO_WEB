(function () {
    angular
        .module('app')
        .controller('PackingListController', PackingListController);

    PackingListController.$inject = [
        '$scope',
        '$state',
        'PackingListService',
        'AlertService',
    ...
    function PackingListController(
        self,
        $state,
        PackingListService,
        AlertService,
        ...
        async function _GetAssetsOnResume() {
            let filters = JSON.parse(localStorage.getItem(ConstService.packingListFilters));
            if (!filters) return;
        ...
        async function _Search(newPage) {
            ...
            localStorage.setItem(ConstService.packingListFilters, JSON.stringify(filters))
            ...
            await _GetPackingAsync(params);
            swal.close();
        }

        async function _GetPackingAsync(params) {
            try {
                self.data = [];
                self.totalItems = 0;
                self.counterData = 0;
                let response = await PackingListService.Get(params);
        ...
        async function _PrintTagAsync(model) {
            try {
                ...
                let dataSend = { 'JsonObject': JSON.stringify(model), 'LabelType': 'packing' }
                let response = await PackingListService.Print(dataSend, adminId);
                const { Status: status, Message: message, Data: data, Counter: counter } = response.data;
                console.log(response);
        ...
        function _ChangueCurrentPage(newPage, oldPage) {
            ...
            _GetPackingAsync(params);
            swal.close();
        }

        function _ClearFilters() {
            localStorage.removeItem(ConstService.packingListFilters);
            self.IdLocation = "";
            self.categoryId = "";
            self.companyId = "";
        }
