(function () {
    ...
    angular
        .module('app')
        .factory('PackingEditService', PackingEditService);

    PackingEditService.$inject = ['$http', 'UrlService'];

    function PackingEditService($http, UrlService) {
        let service = {
            Get: _Get,
            Update: _Update
        };
        ...
        function _Get(id) {
            let url = UrlService.getPackingById.replace("{id}", id)
            let request = {
                method: 'GET',
                url: url,
                async: true
