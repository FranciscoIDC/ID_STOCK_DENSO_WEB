(function () {
    ...
    angular
        .module('app')
        .factory('RampEditService', RampEditService);

    RampEditService.$inject = ['$http', 'UrlService'];

    function RampEditService($http, UrlService) {
        let service = {
            Get: _Get,
            Update: _Update
        };
        ...
        function _Get(id) {
            let url = UrlService.getRampById.replace("{id}", id)
            let request = {
                method: 'GET',
                url: url,
                async: true
