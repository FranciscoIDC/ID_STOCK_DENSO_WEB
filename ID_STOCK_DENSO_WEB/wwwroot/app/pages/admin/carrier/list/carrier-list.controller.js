(function () {
    ...
    function CarrierListController(
        self,
        $state,
        CarrierListService,
        AlertService,
        ...
        async function _GetAssetsOnResume() {
            let filters = JSON.parse(localStorage.getItem(ConstService.carrierListFilters));
            if (!filters) return;
        ...
        async function _Search(newPage) {
            ...
            localStorage.setItem(ConstService.carrierListFilters, JSON.stringify(filters))
            ...
            await _GetCarrierAsync(params);
            swal.close();
        }

        async function _GetCarrierAsync(params) {
            try {
                self.data = [];
                self.totalItems = 0;
                self.counterData = 0;
                let response = await CarrierListService.Get(params);
        ...
        async function _PrintTagAsync(model) {
            try {
                ...
                let dataSend = { 'JsonObject': JSON.stringify(model), 'LabelType': 'carrier' }
                let response = await CarrierListService.Print(dataSend, adminId);
                const { Status: status, Message: message, Data: data, Counter: counter } = response.data;
                console.log(response);
        ...
        function _ChangueCurrentPage(newPage, oldPage) {
            ...
            _GetCarrierAsync(params);
            swal.close();
        }

        function _ClearFilters() {
            localStorage.removeItem(ConstService.carrierListFilters);
            self.IdLocation = "";
            self.categoryId = "";
            self.companyId = "";
        }
