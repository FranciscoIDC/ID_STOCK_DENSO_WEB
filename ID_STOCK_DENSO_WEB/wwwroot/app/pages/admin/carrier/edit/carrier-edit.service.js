(function () {
    ...
    angular
        .module('app')
        .factory('CarrierEditService', CarrierEditService);

    CarrierEditService.$inject = ['$http', 'UrlService'];

    function CarrierEditService($http, UrlService) {
        let service = {
            Get: _Get,
            Update: _Update
        };
        ...
        function _Get(id) {
            let url = UrlService.getCarrierById.replace("{id}", id)
            let request = {
                method: 'GET',
                url: url,
                async: true
